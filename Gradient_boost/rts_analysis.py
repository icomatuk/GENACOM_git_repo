...
def setup(part, inputs, run_folder):
    elements, nodes, baseline_elsets, coordinate_systems = get_mesh(
        inputs["rts_mesh_files"][part], run_folder
    )
...
def get_mesh(mesh_files, run_folder):
    ...
    elements, nodes, baseline_elsets, coordinate_systems = get_2Dmesh(
        mesh_files, run_folder
    )
...
def update_laminate(
    ...
    total_angle = (
        inc_angle + rts_ply["orientations"][laminate["orientation"].upper()]["z_rot_angle"]
...
def write_composite_inputs(
    ...
    for layup_name, layup_val in shell_sections.items():
        ...
        ori = layup_val["orientation"].upper()
...
def check_shell_element_normals(
    ...
    for name, section in shell_sections.items():
        ...
        for e in element_sets[elset]:
            ...
            ori_z = coordinate_systems[section["orientation"].upper()]["R"][2]
            angle = np.rad2deg(
                np.arccos(
...
#     with open("./rts_settings.json", "r") as f:
