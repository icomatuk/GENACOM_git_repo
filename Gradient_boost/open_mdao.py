...
from post_process import post_process_doe
...
def compute(
    ...
    for component in workflow:
        ...
        fd_step = 0.01
        model = prob.model
        has_compute_partials = False  # set this to False if fd gradients should be used
    ...
    print("OpenMDAO problem setup check passed. \n")
    ...
    #     dict_out = run_post(prob, parameters)
        ...
        with open(run_folder / "run_model.log", "w") as f:
...
def run_optimisation(prob, parameters, run_folder):
    ...
    print("Repeat OpenMDAO problem setup for optimisation start. \n")
    prob.setup()
...
def _plot_iteration_histories(
    ...
    for key in inputs_history:
        ...
        for data_series in input_data.T:
            ...
            plt.grid(True)
            plt.title(name)
        plt.savefig(str(run_folder / (name + ".png")))
